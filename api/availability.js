module.exports = async function handler(req, res) {
  ...
  const EVENT_TYPE_ID = process.env.CAL_EVENT_TYPE_ID; // numeric ID from Cal.com
  ...
  try {
    const url = `https://api.cal.com/v1/slots?apiKey=${CAL_API_KEY}&eventTypeId=${EVENT_TYPE_ID}&startTime=${encodeURIComponent(startTime)}&endTime=${encodeURIComponent(endTime)}&timeZone=America/New_York`;
    ...
    // data.slots is an object keyed by date string: { "2025-01-15": [{time: "..."}, ...] }
    const slots = data.slots || {};

    // Flatten into a readable list (up to 8 slots), formatted for voice
    const readable = [];
    for (const [date, times] of Object.entries(slots)) {
      for (const slot of times) {
        if (readable.length >= 8) break;
